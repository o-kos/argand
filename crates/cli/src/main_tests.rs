...
fn paths_reach_stdout_only_when_stdout_is_not_a_terminal() {
    ...
    // into it.
    //
    // The fact itself is passed in rather than read here: a test that let the
    // harness answer `is_terminal()` would pass under `cargo test`, where
    // cargo pipes the child's stdout, and fail for anyone running the test
    // binary from a shell. Covering the call for real needs a pty, which the
    // plan weighs as a dependency it does not take; that one call is checked
    // by hand against the release binary instead.
