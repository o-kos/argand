...
- The application's logic is deliberately free of GPUI wherever it can be, because CI has no GPU and cannot run the application. What a file is doing (`document.rs`), the thread that reads and transforms it (`analysis.rs`), what the settings ask a transform for (`config.rs`), what is remembered between runs (`session.rs`) and where the axes put their marks (the layout half of `axes.rs`) are all tested without a window. Only `shell.rs`, `spectrogram.rs` and the drawing half of `axes.rs` name a toolkit type.
