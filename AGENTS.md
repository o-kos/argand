...
- Linux title-bar controls paint their own rounded hover/press backgrounds; Windows and macOS retain toolkit controls. File uses the same small text size as popup menus. Status metadata fields have separators and contextual tooltips, with duration in minutes:seconds.milliseconds. `document.rs` supplies toolkit-neutral tooltip titles and explanatory sections; `shell.rs` renders terms and descriptions with theme colours.
