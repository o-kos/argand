...
- Planned UI architecture (#124): after #126 rejected the locked stock frame, the owner selected migration through GPUI Kit 0.6.x to its aligned GPUI 0.3.x family (#137), a borderless standard `Root` around the retained Argand frame (#127), focused plot ownership, and maximum reuse of standard controls. #137 and #127 have landed: the graph runs on `gpui-kit` 0.6.x / GPUI 0.3.6, and the production window root is `Root::bordered(false)` strongly owning `Shell` as its child. The compatibility fixture demonstrates the same composition beside the stock-Root baseline. The interactive FFT hint remains separate work in #108.
