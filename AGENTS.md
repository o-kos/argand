...
- `crates/app` opens a signal file, analyses it, and shows the spectrogram with time and frequency axes and a linear waveform above it. There is no zoom or selection yet, and the transform is one blocking pass over the whole file, so a large capture is slow to appear; #29 replaces the inside of that.
