...
- `crates/app` opens a signal file, analyses it, and shows the spectrogram with time and frequency axes and a linear waveform above it. It publishes a sparse full-width preview and paired left-to-right refinement snapshots; time navigation is implemented by #30; selection remains deferred.
...
The shell restores each recent entry's view (session version 6), clamps its span to
...
`navigation_ui.rs` provides wheel zoom about the pointer, left-drag pan, keyboard
