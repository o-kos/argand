...
- The waveform starts at 3 rem (48 logical pixels with the default font), including its bottom separator. A draggable separator persists the adjusted proportion in session version 3; older sessions retain the font-relative default. `panels.rs` bounds the split, while `waveform.rs` draws linear real or separate I/Q envelopes with an explicit `Both` overlap colour, using the exact horizontal geometry measured for the spectrogram. Both arrive from one DSP pass and remain together during replacement analysis. Splitter gestures defer the next analysis request until their final layout after mouse-up. The old `[panels].waveform_fraction` configuration remains readable for compatibility but does not size the panel. Progressive refinement and navigation integration for #31 still depend on #29 and #30.
