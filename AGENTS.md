...
- Planned UI architecture (#124): after #126 rejected the locked stock frame, the owner selected migration through GPUI Kit 0.6.x to its aligned GPUI 0.3.x family (#137), a borderless standard `Root` around the retained Argand frame (#127), focused plot ownership, and maximum reuse of standard controls. The existing unwrapped-shell description below records current code, not the target architecture. Do not add the locked `Root` around the existing frame; it cannot disable its own border. The interactive FFT hint remains separate work in #108.
