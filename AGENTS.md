...
- The B0 application icon trial uses the closed cyan/coral I/Q ribbons. Dedicated 16- and 24-pixel optical PNGs come from `docs/design/app-icon/small-b0-v3/`; larger colored PNGs, ICO and ICNS retain the selected B0 raster sketch. `docs/design/app-icon/install-trial-artwork.py` reproduces this set. The toolbar embeds the 24-pixel icon for its 22-pixel slot. The scalable SVG temporarily embeds the matching raster; a final vector master and updated monochrome/tray artwork remain pending in #41. The desktop installer includes the 24-pixel theme directory.
