...
- `Shell` is the window's root entity and owns theme font setup and Tab traversal. `chrome.rs` provides the single client frame: resize cursors belong to separate edge hitboxes measured against `Window::viewport_size()`, never the restore bounds in `window_bounds()`. Expanded windows have no resize hitboxes; free corners are rounded by 8 logical pixels. Do not wrap the shell in gpui-component's `Root`, which adds its own frame. The transient analysis-settings popup uses one gpui-component `Root` because its standard select and input controls require that window root. Position-capable backends place it above the status control; Wayland retains compositor-managed placement because xdg-shell does not expose absolute toplevel coordinates.
- Linux title-bar controls paint their own rounded hover/press backgrounds; Windows and macOS retain toolkit controls. File uses the same small text size as popup menus. The status bar has a combined file group and an analysis settings popup, with compact duration units in the bar and h:mm:ss.mmm in the file hint. The worker reports file bytes and the reader's optional `original_sample_units` affine conversion (including gain and codec bit depth); complete waveform extrema are converted back to decoded original units, separately for I/Q. Settings and validation live in toolkit-neutral `settings.rs`; `settings_ui.rs` owns the status trigger and popup lifecycle; `settings_editor.rs` renders its standard dropdowns and numeric fields. Ctrl+, (Cmd+, on macOS) opens it, Tab/Shift+Tab traverse controls, arrows/Enter operate them, and Escape closes a dropdown before cancelling the form. Clicking or focusing outside also cancels and restores the opening settings and view. Ctrl+R (Cmd+R on macOS) applies range advice from the main window or editor. Clicking the yellow status-bar dB value dispatches the same action and consumes mouse-down and click so the popup stays closed; the rest of the analysis summary opens the editor. Numeric edits preview on Enter or blur; OK also validates current text in both fields before accepting. `document.rs` supplies toolkit-neutral hint titles, current values and optional explanations; `shell.rs` preserves heading typography, renders aligned file-hint values with muted labels, and uses smaller muted explanations of one sentence without terminal periods. File hints use a viewport-bounded 330-pixel content width with aligned rows. Other metadata hints measure their definite width with GPUI using the rendered font sizes and weights; keep text blocks non-shrinking so wrapped text contributes its full height. The ready-state timing hint describes the latest analysis including sample reads, transforms and CPU image preparation, excluding file opening and window drawing; it is absent for other states.
