...
- Linux CI uses Ubuntu 24.04 and installs GPUI dependencies through `.github/scripts/install-ubuntu-packages.sh`, using only the runner's `ubuntu.sources` and fresh indexes. Signature/hash checks remain enabled; any required-source update error is fatal. Signed local-repository regression tests run before installation.
