...
- The waveform placeholder is fixed at 64 logical pixels, including its bottom separator, and does not shrink during window resizing. The old `[panels].waveform_fraction` setting remains readable for compatibility but no longer sizes it. #31 starts the waveform panel at the same height.
