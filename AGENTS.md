...
- The B0 application icon trial uses the closed cyan/coral I/Q ribbons. The selected raster and 16/24-pixel optical SVG sources live in `crates/app/assets/icons/source/`. `crates/app/assets/icons/generate.py` reproduces the colored PNG, ICO and ICNS set using Pillow and rsvg-convert, with dedicated optical artwork at 16 and 24 pixels. The toolbar embeds the 24-pixel icon for its 22-pixel slot. The scalable SVG temporarily embeds the matching raster; a final vector master and updated monochrome/tray artwork remain pending in #41. The desktop installer includes the 24-pixel theme directory.
