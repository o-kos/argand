...
- Planned UI architecture (#124): the owner selected a standard gpui-component `Root` and its frame, focused plot ownership, and maximum reuse of standard controls. The existing unwrapped-shell description below records current code, not the target architecture. #124 authorizes replacing that restriction as part of a validated single-frame migration; do not add `Root` around the existing custom frame and leave both active. The interactive FFT hint remains separate work in #108.
