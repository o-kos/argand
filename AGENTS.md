...
- The default `theme = "system"` follows GPUI's OS appearance at startup and through a retained main-window appearance subscription. Theme changes refresh the settings popup with the rest of the main window. Explicit `dark` / `light` stay fixed; interface theme does not change the spectral palette.
...
- `Shell` is the window's root entity and owns theme font setup and Tab traversal. `chrome.rs` provides the single client frame: resize cursors belong to separate edge hitboxes measured against `Window::viewport_size()`, never the restore bounds in `window_bounds()`. Expanded windows have no resize hitboxes; free corners are rounded by 8 logical pixels. Do not wrap the shell in gpui-component's `Root`, which adds its own frame. Hover and pinned analysis settings use one `Popover` and one retained `Editor` entity; pinning changes only dismissal policy and never reparents the editor. It uses compact custom controls because gpui-component's standard select and input controls require `Root`.
- Linux title-bar controls paint their own rounded hover/press backgrounds; Windows and macOS retain toolkit controls. File uses the same small text size as popup menus. The status bar has a combined file group and an interactive analysis hint, with compact duration units in the bar and h:mm:ss.mmm in the file hint. The worker reports file bytes and the reader's optional `original_sample_units` affine conversion (including gain and codec bit depth); complete waveform extrema are converted back to decoded original units, separately for I/Q. Settings and validation live in toolkit-neutral `settings.rs`; `settings_ui.rs` owns the status trigger and hint lifecycle; `settings_editor.rs` renders metadata-like rows whose editable values have dashed underlines. Hover creates the one retained editor inside the anchored Popover; moving between the trigger and surface uses a short delayed dismissal. Clicking a value pins that same object and reveals an inline choice list or local numeric stepper, so neither its host, geometry nor inherited typography changes. Fixed numeric width prevents numeric controls from moving. The hint has a compact `Analysis settings` heading and right-aligned Reset command, but no redundant pending row; validation errors replace Reset in the existing footer. The status trigger remains muted on hover like the adjacent file item, except for the yellow range warning. Step buttons act on mouse-down and repeat after 400 ms at 75 ms intervals until mouse-up, including outside release. Ctrl+, (Cmd+, on macOS) pins the hint directly. Tab/Shift+Tab traverse controls, arrows move through choices or step numbers, Space opens or chooses from a list, Enter accepts the complete edit, and Escape restores every setting from when the hint opened. Click-out and focus-out accept the current edit. Preview changes are not saved to the session until acceptance. Reset is a lightweight command rather than part of an OK/Cancel button row. Ctrl+R (Cmd+R on macOS) previews range advice while editing and otherwise applies it directly. Clicking the yellow status-bar dB value dispatches the same action and consumes mouse-down and click so no settings surface opens; the warning is not a hover trigger for settings. Numeric edits preview when leaving the field; invalid text keeps the hint open on Enter. `document.rs` supplies toolkit-neutral hint titles, current values and optional explanations; `shell.rs` preserves heading typography, renders aligned file-hint values with muted labels, and uses smaller muted explanations of one sentence without terminal periods. File hints use a viewport-bounded 330-pixel content width with aligned rows. Other metadata hints measure their definite width with GPUI using the rendered font sizes and weights; keep text blocks non-shrinking so wrapped text contributes its full height. The ready-state timing hint describes the latest analysis including sample reads, transforms and CPU image preparation, excluding file opening and window drawing; it is absent for other states.
...
reapply this floor before requesting analysis. The settings hint owns the
opening settings for Escape rollback but no navigation snapshot: previews and
any resulting view bounding apply immediately.
