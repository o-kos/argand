...
- Planned UI architecture (#124): after #126 rejected the locked stock frame, the owner selected migration through GPUI Kit 0.6.x to its aligned GPUI 0.3.x family (#137), a borderless standard `Root` around the retained Argand frame (#127), focused plot ownership, and maximum reuse of standard controls. #137 has landed: the graph runs on `gpui-kit` 0.6.x / GPUI 0.3.6, and the compatibility fixture demonstrates `Root::bordered(false)` in a second window beside the stock one. The borderless production Root and frame ownership remain #127. The existing unwrapped-shell description below records current code, not the target architecture. Do not wrap the production frame in a `Root` before #127; it cannot disable its own border. The interactive FFT hint remains separate work in #108.
...
- Pre-#124 implementation (descriptive, not a prohibition on the approved migration): `Shell` is currently the window's root entity and owns theme font setup and Tab traversal. `chrome.rs` provides the single client frame: resize cursors belong to separate edge hitboxes measured against `Window::viewport_size()`, never the restore bounds in `window_bounds()`. Expanded windows have no resize hitboxes; free corners are rounded by 8 logical pixels. The main shell is currently unwrapped to avoid duplicating this frame with gpui-component's `Root`; #137 migrated the stack to GPUI Kit 0.6.x / GPUI 0.3.x and proved `Root::bordered(false)` in the fixture, and #127 places the retained frame under it. The separate analysis settings window uses one gpui-component `Root` for standard select/input focus, Tab traversal and its own frame.
