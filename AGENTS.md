...
- `crates/app` opens a signal file, analyses it, and shows the spectrogram with time and frequency axes. There is no waveform panel, no zoom and no selection yet, and the transform is one blocking pass over the whole file, so a large capture is slow to appear; #29 replaces the inside of that.
...
- The waveform placeholder is 3 rem high (48 logical pixels with the default font), including its bottom separator, and does not shrink during window resizing. The old `[panels].waveform_fraction` setting remains readable for compatibility but no longer sizes it. #31 starts the waveform panel at the same height.
