...
the spectrogram carries a translucent unrounded `[+|-]` pair in its leading and
trailing corners: the bottom-left pair zooms time, the top-right pair frequency
(mirrored in vertical orientation). Both dispatch the registered zoom actions,
show the corresponding keyboard shortcut on hover, and disappear on small
spectrograms.
...
Ctrl+Shift+`0` fits frequency using either top-row or keypad zero. Some Windows
IME/language-switching configurations intercept this shortcut; use View → Fit
frequency when it does not reach the application.
