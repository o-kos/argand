...
- `crates/app/assets/icons/generate.py`: reproduces the colored PNG, ICO, ICNS
  and scalable SVG assets using Pillow and rsvg-convert.
...
- [ ] Produce the full-size vector master and update monochrome/tray artwork.
- [ ] Complete supported-platform delivery validation.
- [ ] Move this plan to completed before final review.

## Rendering and delivery

16 and 24 pixels use the optical SVG redraws; larger sizes retain the selected
raster. The scalable SVG temporarily embeds that raster, so desktop theme lookup
uses the same design; it is not the pending full-size vector master. The desktop
...
The focused review of the final exploration removal found no substantive issues:
