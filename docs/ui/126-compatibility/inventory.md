...
| --- | --- | --- | --- | --- |
| C01 | [chrome.rs](../../../crates/app/src/chrome.rs), `Frame::for_window/render` | Custom client frame, shadow, inset and edge hitboxes; current production does not exhibit the owner-reported resize failures | Retain as the sole frame under GPUI Kit's `Root::bordered(false)` after #137 | R1/R2: standard fixture had stale/displaced resize cursor and impractical diagonal target; migration must not regress the current behavior or add a second frame layer |
| C02 | `chrome::controls` | Custom div minimize/maximize/close with global secondary hover/active colors; maximize toggles on each click | Per-click double-click behavior is owner-accepted; correct contrast through the shared palette | R3/B1: validate global semantic colors across affected controls; do not retain custom controls solely for caption contrast |
| C03 | [shell.rs](../../../crates/app/src/shell.rs), `title_bar` | Linux move/release/outside handlers and context window menu; current bare-title double-click works | Retain accepted Argand behavior through #137/#127 | Standard fixture bare-title double-click was unreliable; toolbar hit testing must prevent unintended move/maximize |
...
| Stock window controls are private; Linux maximize/restore calls zoom_window on every click | `title_bar.rs`, `ControlIcon` and `WindowControls` | The owner accepts the double-click sequence; it is recorded behavior, not a #127 blocker |
