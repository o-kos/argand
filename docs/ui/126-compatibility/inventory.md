...
| --- | --- | --- | --- | --- |
| C01 | [chrome.rs](../../../crates/app/src/chrome.rs), `Frame::for_window/render` | Custom client frame, shadow, inset and edge hitboxes; current production does not exhibit the owner-reported resize failures | Standard Root frame is NO-GO pending supported upstream repair | R1/R2: standard fixture had stale/displaced resize cursor and impractical diagonal target; stock private frame uses reported bounds, so migration must not regress the current behavior |
| C02 | `chrome::controls` | Custom div minimize/maximize/close with global secondary hover/active colors; maximize toggles on each click | Standard TitleBar does not fix the per-click behavior; evaluate shared palette separately | R3/B1: current maximize double-click toggles twice; dark contrast may be corrected through global semantic tokens after whole-interface validation |
| C03 | [shell.rs](../../../crates/app/src/shell.rs), `title_bar` | Linux move/release/outside handlers and context window menu; current bare-title double-click works | Reassess only after the standard frame gate has a supported candidate | Standard fixture bare-title double-click was unreliable; toolbar hit testing must prevent unintended move/maximize |
...
| Stock window controls are private; Linux maximize/restore calls zoom_window on every click | `title_bar.rs`, `ControlIcon` and `WindowControls` | The application cannot suppress the second click while retaining the standard control; owner-observed double transition blocks #127 |
| Caption-control hover/active uses global secondary tokens with no per-control public style hook | `title_bar.rs`, `ControlIcon::render`; production `chrome.rs` | A coherent global palette adjustment can fix contrast in both implementations, but requires regression checks for every secondary-token consumer |
...
| Root dialog layer is not deferred; open/close_dialog save and restore prior focus | `root.rs`, render_dialog_layer and WindowExt; `dialog.rs`, RenderOnce | Test a standard Dialog as a separate modal candidate, not as an approved UX-equivalent replacement for an anchored editor. No dependency or custom-control workaround |
