...
| --- | --- | --- | --- | --- |
| C01 | [chrome.rs](../../../crates/app/src/chrome.rs), `Frame::for_window/render` | Custom client frame, shadow, inset and edge hitboxes | Standard Root frame is NO-GO pending supported upstream repair | R1/R2: owner reports stale/displaced resize cursor and impractical diagonal target; stock private frame uses reported bounds, so exact viewport/geometry evidence is still required |
| C02 | `chrome::controls` | Custom div minimize/maximize/close with hover/active backgrounds | Standard TitleBar controls are NO-GO pending supported upstream repair | R3/B1: owner reports double-click maximize/restore and indistinct dark hover/pressed; private controls expose neither action nor per-control style hooks |
| C03 | [shell.rs](../../../crates/app/src/shell.rs), `title_bar` | Linux move/release/outside handlers and context window menu; stock TitleBar on other platforms | Reassess only after the frame gate has a supported candidate | Bare-title double-click must toggle once and reliably; toolbar hit testing must prevent unintended move/maximize |
...
| Caption-control hover/active uses global secondary tokens with no per-control public style hook | `title_bar.rs`, `ControlIcon::render` | Do not alter global control colors to disguise a TitleBar limitation; dark-theme visual acceptance blocks #127 |
