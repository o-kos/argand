...
def run(label,workers=8,affinity='none',mask=None,round=0):
 case=base/f'ui-{label}-r{round}';case.mkdir(exist_ok=True)
 ...
 with (case/'client.log').open('w') as log:
  ...
  masks={};lag=[];complete_at=None;tick=0
  while time.monotonic()-started<90 and app.poll() is None:
   ...
   if 'analysis completed' in data and complete_at is None:complete_at=time.monotonic()
...
if __name__=='__main__':
 if len(sys.argv)>1 and sys.argv[1]=='smoke':run('optimized',round=-1)
 else:
