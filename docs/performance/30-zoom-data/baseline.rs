...
fn main() {
 ...
 let mut state=analyze_overview(source.as_mut(),&request,ProgressiveOptions::default(),&||Flow::Continue,&mut |s,_|{if publish {black_box(s.render(1500,700,Some(1500)).unwrap());renders+=1;} Flow::Continue}).unwrap();
 black_box(state.render(1500,700,Some(1500)).unwrap());times.push(start.elapsed().as_secs_f64()*1000.);
 }
